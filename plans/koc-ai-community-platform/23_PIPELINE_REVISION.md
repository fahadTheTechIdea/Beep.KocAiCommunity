...
**Date:** 2026-07-27
**Status:** 🟢 23a–23f DONE — the node graph is the single source of truth for control facts (executor reads target/id/task from the Train node; a passed value overrides); the primary Dataset node no longer collides with the secondary scanner; competition submit resolves the participant's join/union datasets; Execute/Predict share the id-exclusion; the designer validates the graph inline; and a submitted pipeline has a hard time budget enforced at node boundaries. **23g DONE** — direct-upload id-coverage (submissions missing any answer-key id are rejected) and **quota-race atomicity** (the daily-quota count + submission insert now run inside a serializable transaction in `ScoreAndRecordAsync`, so two concurrent submits can't both slip past the cap — no migration needed; no retrying execution strategy is configured, so a user-initiated transaction is safe). Phase 23 is complete.
