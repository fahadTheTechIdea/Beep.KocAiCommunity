...
**Category:** Transform · **Ports:** Table → Table · **Handler:** `StandardizeHandler` (`src/Beep.KocAiCommunity.ML/Nodes/MlTransformHandlers.cs`)

Rescales numeric features to mean 0 and variance 1.

## What it does
1. Selects all `NumericFeatures` (Single-typed feature columns, excluding label/id/`__fold`).
2. Applies `NormalizeMeanVariance` to those columns in place.
...
- Add an optional `columns` picker (blank = all numeric) so users can standardize a subset instead of every numeric feature.

## Notes
- Runs through `FitTransform` — fit on TRAIN FOLD ONLY plus recorded replay, so mean/variance come only from training data (no leakage).
