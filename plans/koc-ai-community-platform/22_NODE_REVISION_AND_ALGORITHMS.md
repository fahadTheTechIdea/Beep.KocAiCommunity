...
**Date:** 2026-07-27
**Status:** 🟢 22a–22e DONE — every node documented (37 docs); column pickers + friendly dropdowns; Train exposes target/id/task (competition pre-fills & locks); per-algorithm hyperparameters + new trainers (GAM/SGD/OGD/One-vs-all) proven to train; transform friendliness (scaler/encoder column selectors, hash `bits`, one-hot `outputKind`); join-dataset join-type dropdown + key picker; and the visual builders — **aggregate builder** (group-by), **sort-key builder** (sort), **condition builder** (sql-filter) — that generate the SQL into the existing config keys, with the raw text kept as an advanced fallback.
